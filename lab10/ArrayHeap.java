...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        if (index == 1) {
            return;
        } else if (getNode(index).priority()
    ...
    private void sink(int index) {
        ...
        if (!inBounds(leftIndex(index))) {
            return;
        } else if (getNode(index).priority() > getNode(min(leftIndex(index),
                rightIndex(index))).priority()) {
    ...
    /**
     * Inserts an item with the given priority value. This is enqueue,
     * or offer. To implement this method, add it to the end of the ArrayList,
     * then swim it.
     * @param item to be inserted
     * @param priority to be inserted
     */
    @Override
    ...
    /**
     * Returns the Node with the smallest priority value, but does not remove
     * it from the heap. To implement this, return the item in the 1st position
     * of the ArrayList.
     */
    @Override
    public T peek() {
     ...
     * @param item key to search in the heap
    ...
    public void changePriority(T item, double priority) {
        ...
        getNode(target).myPriority = priority;
        if (target == 1) {
            sink(target);
            return;
        }

        if (!inBounds(leftIndex(target)) && !inBounds(rightIndex(target))) {
            swim(target);
            return;
        }

        if (priority < getNode(parentIndex(target)).priority()) {
            swim(target);
        } else if (priority > getNode(min(leftIndex(target),
                rightIndex(target))).priority()) {
            sink(target);
        } else {
            return;
        }
    ...
    /**
     * Prints out the heap sideways. Provided for you. Return the string
     * representation.
     */
    @Override
    ...
    private class Node {
        /** The item stored. */
        private T myItem;
        /** The corresponding priority of the item. */
        ...
        /** Return the priority in the Node. */
        public double priority() {
            return myPriority;
        }

        /** Transform the heap into string. */
        @Override
        public String toString() {
     ...
     * @param capacity of present array
     */
